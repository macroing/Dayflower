...
public final class Camera {
	...
	public Camera() {
		this(Matrix44F.perspective(AngleF.degrees(70.0F), 640.0F / 480.0F, 0.1F, 1000.0F));
	}
	
	public Camera(final Matrix44F projection) {
		this.projection = projection;
		this.screenSpaceTransform = Matrix44F.screenSpaceTransform(640.0F, 480.0F);
	...
	public Point2I getProjectedPoint(final float x, final float y, final float z) {
		final Point4F p = Point4F.transform(getViewProjection(), new Point4F(x, y, z));
		
		final boolean isInsideViewFrustumX = abs(p.x) <= abs(p.w);
		final boolean isInsideViewFrustumY = abs(p.y) <= abs(p.w);
		final boolean isInsideViewFrustumZ = abs(p.z) <= abs(p.w);
		
		if(isInsideViewFrustumX && isInsideViewFrustumY && isInsideViewFrustumZ) {
			final Point4F q = Point4F.transformAndDivide(this.screenSpaceTransform, p);
			
			return new Point2I((int)(q.x), (int)(q.y));
		}
